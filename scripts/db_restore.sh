...
show_help() {
    echo "Usage: ./db-tools.sh restore [OPTIONS]"
    echo ""
    ...
    echo ""
    echo "Example:"
    echo "  ./db-tools.sh restore -u root -p secret -h localhost -P 3306 -d mydb -f backup.sql"
    echo "  ./db-tools.sh restore --user=root --password=secret --database=mydb --file=backup.sql"
}
